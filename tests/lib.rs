...
fn multihash_methods<H, const S: usize>(
  code: Code,
  prefix: &str,
  digest_str: &str,
) where
  ...
  Code: for<'a> From<&'a H::Digest>,
{
  ...
  let expected_bytes =
    hex::decode(&format!("{}{}", prefix, digest_str)).unwrap();
...
#[test]
fn multihash_errors() {
  assert!(Multihash::from_bytes(&[]).is_err(), "Should error on empty data");
  assert!(
  ...
  assert!(
    Multihash::from_bytes(&[identity_code, identity_length, 1, 2, 3, 4])
      .is_err(),
